...
URL = "https://www2.futureware.at/~philippe/SiliconMirror/softdown/soft/201306/53BK_v6.3_jb51.rar"
UA = "Mozilla/5.0 qilu-shaonian-53bk-focused/1.0"
...
# Capture quoted path/URL-like literals, not complete source code.
QUOTED = re.compile(r'''["']([^"'\r\n]{1,260})["']''')
...
def main() -> int:
    raw = fetch()
    if raw[:2] != b"PK":
        raise RuntimeError("reference package is not ZIP-formatted")
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        for name in names:
            ...
            # Extract short contexts around the first few high-signal tokens.
            seen_ctx = set()
            ...
            # Extract quoted literals that look like routes, folders, filenames or endpoints.
            seen_lit = set()
            for q in QUOTED.finditer(text):
                value = q.group(1).strip()
                if not value or not PATH_SIGNAL.search(value):
                    continue
                if value in seen_lit:
                    continue
                seen_lit.add(value)
    ...
    with (OUT / "focused_entries.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["entry", "kind"])
        w.writeheader(); w.writerows(entry_rows)
    with (OUT / "focused_literals.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["entry", "literal"])
        w.writeheader(); w.writerows(literal_rows)
    with (OUT / "focused_contexts.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["entry", "token", "context"])
        w.writeheader(); w.writerows(contexts[:600])
    ...
    print(json.dumps({k: report[k] for k in ("package_entries","focused_filename_matches","focused_quoted_literals","focused_contexts")}, ensure_ascii=False, indent=2), flush=True)
    return 0
